...
@st.cache_data(ttl=3600)  # Step 1: Prevent Rate Limits (1-hour cache)
...
def run_monte_carlo(start_price, sd, days=21, sims=100):
    # Quant Logic: Projecting future volatility paths
    ...
    for i in range(sims):
        prices = [start_price]
        for _ in range(days):
            # Random walk based on Normal Distribution of current Volatility
...
st.sidebar.divider()
st.sidebar.caption("v4.0 | Predictive Monte Carlo Enabled")
...
if ticker:
    ...
    if not df.empty and len(df) >= 50:
        ...
        c2.metric("Volatility (SD)", f"±${curr_sd:.2f}")
        ...
        # Shaded Neutral Zone (The Noise Floor)
        ...
        # 4. STEP 1: PREDICTIVE MONTE CARLO LAYER
        st.divider()
        col_left, col_right = st.columns([2, 1])
        
        with col_left:
            st.markdown("### 21-Day Risk Projection")
            st.caption("Monte Carlo simulation based on current volatility and sensitivity settings.")
        ...
        with col_right:
            ...
            st.write(f"**Confidence:** {abs(precise_score - 5) * 20}%")
            ...
            *The projection cloud represents the 'Statistical Possible Range' based on the last 50 days of volatility.*
            """)

    else:
        st.error("Incomplete market data. Please verify the ticker symbol.")
